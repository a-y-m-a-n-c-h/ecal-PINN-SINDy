...
class ECAL_Dataset_Prep:
    def __init__(self, csv_file, input_len, output_len, stride, fig_name_cali, fig_name_scaled_cali, verbose=False, lumi_threshold=0.0):
        ...
        self.scaler_deltat = StandardScaler()
    ...
    def get_df(self):
        ...
        try:
            ...
            self.df_deltat = self.df[['deltat']].copy()
            
            if DEBUG:
                # TODO: Problem with normalization
                lumi_threshold = self.lumi_threshold # max(self.df_lumi['delta_lumi'])*0.0
                mask = self.df_lumi['delta_lumi'] > self.lumi_threshold
                self.df_cali = self.df_cali[mask]
                self.df_lumi = self.df_lumi[mask]
                self.df_deltat = self.df_deltat[mask]
                pass
    ...
    def normalize_dataset(self):
        ...
        if len(self.df_cali) !=0:
            ...
            self.np_cali_unscaled = self.df_cali['calibration'].to_numpy()
            self.np_cali_unscaled = self.np_cali_unscaled.reshape(-1,1)
        ...
        if len(self.df_lumi) !=0:
            ...
            self.np_lumi_unscaled = self.df_lumi['delta_lumi'].to_numpy()
            self.np_lumi_unscaled = self.np_lumi_unscaled.reshape(-1,1)

        # normalize deltat
        if len(self.df_deltat) != 0:
            self.scaler_deltat.fit(self.df_deltat[['deltat']])
            self.df_deltat['deltat_scaled'] = None
            self.df_deltat.loc[:,'deltat_scaled'] = self.scaler_deltat.transform(self.df_deltat[['deltat']])
            if self.verbose:
                print(self.df_deltat.describe())
            self.np_deltat = self.df_deltat['deltat_scaled'].to_numpy()
            self.np_deltat = self.np_deltat.reshape(-1, 1)
            self.np_deltat_unscaled = self.df_deltat['deltat'].to_numpy()
            self.np_deltat_unscaled = self.np_deltat_unscaled.reshape(-1,1)
            # print(self.df_deltat['deltat'])
            # print(self.df_deltat['deltat_scaled'])
    ...
    def sequence_dataset(self):
        ...
        num_deltat = self.np_deltat.shape[0]
        ...
        if num_lumi == num_cali and num_lumi!=0:
            ...
            y1_combined = np.hstack((self.np_lumi, self.np_cali,self.np_deltat,self.np_lumi_unscaled, self.np_cali_unscaled,self.np_deltat_unscaled))
