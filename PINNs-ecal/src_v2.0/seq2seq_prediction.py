...
class Seq2Seq_Prediction:
    ...
    def prediction_case1(self):
        ...
        for ii in range(batches):
            # train set
            X_train_temp = self.Xtrain[:, ii, :3]
            Y_train_temp = self.Ytrain[:, ii, :3]
    ...
    def predict(self, encoder, decoder, input_tensor, target_tensor, target_len):
        ...
        with torch.no_grad():
            ...
            target_tensor_input2 = target_tensor_2features[:, :, 2:3]
            # target_tensor is the "calibration", which is the value we want to predict
            target_tensor = target_tensor_2features[:, :, 1:2]
            ...
            for t in range(target_len):
                ...
                deltat_feature = target_tensor_input2[t,:,:]
    ...
    def plot_prediction(self, target, pred, lumi_info, time_info, fig_name, fig_title):
        ...
        plot1 = ax1.plot(time_info, target, color='b', linewidth=2, label='Calibration (true)', linestyle='None', marker = 'o', markersize = 4)
        plot2 = ax1.plot(time_info, pred, color='r', linewidth=2, label='Calibration (prediction)', linestyle='None', marker = 'o', markersize = 4)
        ...
        ### now, start the other y plots
        ax2 = ax1.twinx()
        plot3 = ax2.plot(time_info, lumi_info, label='Luminosity', color='grey', linewidth=1, linestyle='None', marker = 'o', markersize = 1)
